...
/**
 * ClassName: ClusterSlotHashUtil
 * Description:
 * Date: 2023/04/23 AM 10:21:18
 * <p>
 * project: jedis-demo
 * package: com.heima.jedis.util
...
public class ClusterSlotHashUtil {
    ...
    private static final byte SUB_KEY_START = '{';
    private static final byte SUB_KEY_END = '}';
    ...
    public static int calculateSlot(byte[] key) {
        ...
        byte[] finalKey = key;
        int start = indexOf(key, SUB_KEY_START);
        if (start != -1) {
            int end = indexOf(key, start + 1, SUB_KEY_END);
