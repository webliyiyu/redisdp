 ...
 * @Date: 2023-04-19 13:52:40
 * @LastEditTime: 2023-04-19 16:35:11
 * @LastEditors: Guojian Wang 1085844536@qq.com
 * @FilePath: \READMEd:\Developer\WGJ\Java_WorkSpace\redis\doc\02-实战篇\02.实战篇.md
 * @Description:
-->
# 实战篇Redis
...
- [实战篇Redis](#实战篇redis)
  ...
  - [开篇导读](#开篇导读)
  - [1、短信登录](#1短信登录)
    - [1.1、导入黑马点评项目](#11导入黑马点评项目)
      - [1.1.1 、导入SQL](#111-导入sql)
      - [1.1.2、有关当前模型](#112有关当前模型)
      - [1.1.3、导入后端项目](#113导入后端项目)
      - [1.1.4、导入前端工程](#114导入前端工程)
      - [1.1.5、运行前端项目](#115运行前端项目)
    - [1.2 、基于Session实现登录流程](#12-基于session实现登录流程)
    - [1.3 、实现发送短信验证码功能](#13-实现发送短信验证码功能)
    - [1.4、实现登录拦截功能](#14实现登录拦截功能)
    - [1.5、隐藏用户敏感信息](#15隐藏用户敏感信息)
    - [1.6、session共享问题](#16session共享问题)
    - [1.7 Redis代替session的业务流程](#17-redis代替session的业务流程)
      - [1.7.1、设计key的结构](#171设计key的结构)
      - [1.7.2、设计key的具体细节](#172设计key的具体细节)
      - [1.7.3、整体访问流程](#173整体访问流程)
    - [1.8 基于Redis实现短信登录](#18-基于redis实现短信登录)
    - [1.9 解决状态登录刷新问题](#19-解决状态登录刷新问题)
      - [1.9.1 初始方案思路总结](#191-初始方案思路总结)
      - [1.9.2 优化方案](#192-优化方案)
      - [1.9.3 代码](#193-代码)
  - [2、商户查询缓存](#2商户查询缓存)
    - [2.1 什么是缓存?](#21-什么是缓存)
      - [2.1.1 为什么要使用缓存](#211-为什么要使用缓存)
      - [2.1.2 如何使用缓存](#212-如何使用缓存)
    - [2.2 添加商户缓存](#22-添加商户缓存)
      - [2.2.1 、缓存模型和思路](#221-缓存模型和思路)
      - [2.1.2、代码如下](#212代码如下)
    - [2.3 缓存更新策略](#23-缓存更新策略)
      - [2.3.1 、数据库缓存不一致解决方案](#231-数据库缓存不一致解决方案)
      - [2.3.2 、数据库和缓存不一致采用什么方案](#232-数据库和缓存不一致采用什么方案)
    - [2.4 实现商铺和缓存与数据库双写一致](#24-实现商铺和缓存与数据库双写一致)
    - [2.4 实现商铺和缓存与数据库双写一致](#24-实现商铺和缓存与数据库双写一致-1)
    - [2.5 缓存穿透问题的解决思路](#25-缓存穿透问题的解决思路)
    - [2.6 编码解决商品查询的缓存穿透问题](#26-编码解决商品查询的缓存穿透问题)
    - [2.7 缓存雪崩问题及解决思路](#27-缓存雪崩问题及解决思路)
    - [2.8 缓存击穿问题及解决思路](#28-缓存击穿问题及解决思路)
    - [2.9 利用互斥锁解决缓存击穿问题](#29-利用互斥锁解决缓存击穿问题)
    - [3.0 、利用逻辑过期解决缓存击穿问题](#30-利用逻辑过期解决缓存击穿问题)
    - [3.1、封装Redis工具类](#31封装redis工具类)
  - [3、优惠卷秒杀](#3优惠卷秒杀)
    - [3.1 -全局唯一ID](#31--全局唯一id)
    - [3.2 -Redis实现全局唯一Id](#32--redis实现全局唯一id)
    - [3.3 添加优惠卷](#33-添加优惠卷)
    - [3.4 实现秒杀下单](#34-实现秒杀下单)
    - [3.5 库存超卖问题分析](#35-库存超卖问题分析)
    - [3.6 乐观锁解决超卖问题](#36-乐观锁解决超卖问题)
    - [3.6 优惠券秒杀-一人一单](#36-优惠券秒杀-一人一单)
    - [3.7 集群环境下的并发问题](#37-集群环境下的并发问题)
  - [4、分布式锁](#4分布式锁)
    - [4.1 、基本原理和实现方式对比](#41-基本原理和实现方式对比)
    - [4.2 、Redis分布式锁的实现核心思路](#42-redis分布式锁的实现核心思路)
    - [4.3 实现分布式锁版本一](#43-实现分布式锁版本一)
    - [4.4 Redis分布式锁误删情况说明](#44-redis分布式锁误删情况说明)
    - [4.5 解决Redis分布式锁误删问题](#45-解决redis分布式锁误删问题)
    - [4.6 分布式锁的原子性问题](#46-分布式锁的原子性问题)
    - [4.7 Lua脚本解决多条命令原子性问题](#47-lua脚本解决多条命令原子性问题)
    - [4.8 利用Java代码调用Lua脚本改造分布式锁](#48-利用java代码调用lua脚本改造分布式锁)
  - [5、分布式锁-redission](#5分布式锁-redission)
    - [5.1 分布式锁-redission功能介绍](#51-分布式锁-redission功能介绍)
    - [5.2 分布式锁-Redission快速入门](#52-分布式锁-redission快速入门)
    - [5.3 分布式锁-redission可重入锁原理](#53-分布式锁-redission可重入锁原理)
    - [5.4 分布式锁-redission锁重试和WatchDog机制](#54-分布式锁-redission锁重试和watchdog机制)
    - [5.5 分布式锁-redission锁的MutiLock原理](#55-分布式锁-redission锁的mutilock原理)
    - [5.6 总结](#56-总结)
  - [6、秒杀优化](#6秒杀优化)
    - [6.1 秒杀优化-异步秒杀思路](#61-秒杀优化-异步秒杀思路)
    - [6.2 秒杀优化-Redis完成秒杀资格判断](#62-秒杀优化-redis完成秒杀资格判断)
    - [6.3 秒杀优化-基于阻塞队列实现秒杀优化](#63-秒杀优化-基于阻塞队列实现秒杀优化)
  - [7、Redis消息队列](#7redis消息队列)
    - [7.1 Redis消息队列-认识消息队列](#71-redis消息队列-认识消息队列)
    - [7.2 Redis消息队列-基于List实现消息队列](#72-redis消息队列-基于list实现消息队列)
    - [7.3 Redis消息队列-基于PubSub的消息队列](#73-redis消息队列-基于pubsub的消息队列)
    - [7.4 Redis消息队列-基于Stream的消息队列](#74-redis消息队列-基于stream的消息队列)
    - [7.5 Redis消息队列-基于Stream的消息队列-消费者组](#75-redis消息队列-基于stream的消息队列-消费者组)
    - [7.6 基于Redis的Stream结构作为消息队列，实现异步秒杀下单](#76-基于redis的stream结构作为消息队列实现异步秒杀下单)
  - [8、达人探店](#8达人探店)
    - [8.1、达人探店-发布探店笔记](#81达人探店-发布探店笔记)
    - [8.2 达人探店-查看探店笔记](#82-达人探店-查看探店笔记)
    - [8.3 达人探店-点赞功能](#83-达人探店-点赞功能)
    - [8.4 达人探店-点赞排行榜](#84-达人探店-点赞排行榜)
  - [9、好友关注](#9好友关注)
    - [9.1 好友关注-关注和取消关注](#91-好友关注-关注和取消关注)
    - [9.2 好友关注-共同关注](#92-好友关注-共同关注)
    - [9.3 好友关注-Feed流实现方案](#93-好友关注-feed流实现方案)
    - [9.4 好友关注-推送到粉丝收件箱](#94-好友关注-推送到粉丝收件箱)
    - [9.5好友关注-实现分页查询收邮箱](#95好友关注-实现分页查询收邮箱)
  - [10、附近商户](#10附近商户)
    - [10.1、附近商户-GEO数据结构的基本用法](#101附近商户-geo数据结构的基本用法)
    - [10.2、 附近商户-导入店铺数据到GEO](#102-附近商户-导入店铺数据到geo)
    - [10.3 附近商户-实现附近商户功能](#103-附近商户-实现附近商户功能)
  - [11、用户签到](#11用户签到)
    - [11.1、用户签到-BitMap功能演示](#111用户签到-bitmap功能演示)
    - [11.2 、用户签到-实现签到功能](#112-用户签到-实现签到功能)
    - [11.3 用户签到-签到统计](#113-用户签到-签到统计)
      - [11.4 额外加餐-关于使用bitmap来解决缓存穿透的方案](#114-额外加餐-关于使用bitmap来解决缓存穿透的方案)
  - [12、UV统计](#12uv统计)
    - [12.1 、UV统计-HyperLogLog](#121-uv统计-hyperloglog)
    - [12.2 UV统计-测试百万数据的统计](#122-uv统计-测试百万数据的统计)
...
**缓存**(Cache),就是数据交换的**缓冲区**,俗称的缓存就是**缓冲区内的数据**,一般从数据库中获取,存储于本地代码例如:
...
第二个难点是由于我们校验和下单是两个tomcat线程，那么我们如何知道到底哪个单他最后是否成功，或者是下单完成，为了完成这件事我们在redis操作完之后，我们会将一些信息返回给前端，同时也会把这些信息丢到异步queue中去，后续操作中，可以通过这个id来查询我们tomcat中的下单逻辑是否完成了。
...
<div align="center">
    <img src="./assets/90.png" alt=90>
</div>

在业务开发中，我们可以循环的调用XREAD阻塞方式来查询最新消息，从而实现持续监听队列的效果，伪代码如下

<div align="center">
    <img src="./assets/91.png" alt=91>
</div>
...
<div align="center">
    <img src="./assets/92.png" alt=92>
</div>

创建消费者组：

<div align="center">
    <img src="./assets/93.png" alt=93>
</div>
...
<div align="center">
    <img src="./assets/101.png" alt=101>
</div>
...
<div align="center">
    <img src="./assets/102.png" alt=102>
</div>
...
<div align="center">
    <img src="./assets/103.png" alt=103>
</div>
...
<div align="center">
    <img src="./assets/104.png" alt=104>
</div>
...
<div align="center">
    <img src="./assets/105.png" alt=105>
</div>
...
<div align="center">
    <img src="./assets/106.png" alt=106>
</div>

对于新型的Feed流的的效果：不需要我们用户再去推送信息，而是系统分析用户到底想要什么，然后直接把内容推送给用户，从而使用户能够更加的节约时间，不用主动去寻找。

<div align="center">
    <img src="./assets/107.png" alt=107>
</div>
...
<div align="center">
    <img src="./assets/108.png" alt=108>
</div>
...
<div align="center">
    <img src="./assets/109.png" alt=109>
</div>
...
<div align="center">
    <img src="./assets/110.png" alt=110>
</div>
...
<div align="center">
    <img src="./assets/111.png" alt=111>
</div>
...
<div align="center">
    <img src="./assets/112.png" alt=112>
</div>
...
<div align="center">
    <img src="./assets/113.png" alt=113>
</div>
...
<div align="center">
    <img src="./assets/114.png" alt=114>
</div>

当我们点击美食之后，会出现一系列的商家，商家中可以按照多种排序方式，我们此时关注的是距离，这个地方就需要使用到我们的GEO，向后台传入当前app收集的地址(我们此处是写死的) ，以当前坐标作为圆心，同时绑定相同的店家类型type，以及分页信息，把这几个条件传入后台，后台查询出对应的数据再返回。

<div align="center">
    <img src="./assets/115.png" alt=115>
</div>
...
<div align="center">
    <img src="./assets/116.png" alt=116>
</div>
...
<div align="center">
    <img src="./assets/117.png" alt=117>
</div>
...
<div align="center">
    <img src="./assets/118.png" alt=118>
</div>
...
<div align="center">
    <img src="./assets/119.png" alt=119>
</div>
...
<div align="center">
    <img src="./assets/120.png" alt=120>
</div>
...
<div align="center">
    <img src="./assets/121.png" alt=121>
</div>
...
<div align="center">
    <img src="./assets/122.png" alt=122>
</div>
...
<div align="center">
    <img src="./assets/123.png" alt=123>
</div>
...
<div align="center">
    <img src="./assets/124.png" alt=124>
</div>
